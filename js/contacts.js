...
async function addContact(event) {
    ...
    if (!name || name.length < 2) {
        await showAlert('Name must be at least 2 characters long', 'error');
        return;
    }
    ...
    if (!phone) {
        await showAlert('Phone number is required', 'error');
        return;
    }
    ...
    if (existingContact) {
        await showAlert('A contact with this email already exists', 'error');
        return;
    }
    ...
    try {
        ...
        await showAlert('Contact successfully created!', 'success');
        
    } catch (error) {
        console.error('Error creating contact:', error);
        await showAlert('Error creating contact', 'error');
    }
}
...
async function saveEditContact(event) {
    ...
    try {
        ...
        if (updatedContact) {
            ...
            await showAlert('Contact successfully edited!', 'success');
        } else {
            await showAlert('Error updating contact', 'error');
        }
...
async function deleteContactData() {
    ...
    try {
        ...
        if (result) {
            ...
            await showAlert('Contact deleted successfully!', 'success');
        } else {
            await showAlert('Error deleting contact', 'error');
        }
